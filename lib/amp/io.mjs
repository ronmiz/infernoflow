...
export function toAmp(internal) {
  ...
  // detailRef — pointer to a sidecar body file (details/<id>.md) holding the
  // rich Tier-2 content. Kept in meta so the wire stays AMP-spec-compliant; the
  // body itself NEVER rides the JSONL (that's the whole point — a lean index).
  if (internal.detailRef) meta.detailRef = internal.detailRef;
...
export function fromAmp(amp) {
  ...
  if (meta.detailRef) internal.detailRef = meta.detailRef;     // Tier-2 sidecar pointer
  ...
  const { subtype, agent, result, detailRef, ...restMeta } = meta;
 ...
 */
const _migratedRoots = new Set();

/** Absolute path to the consolidated Tier-2 detail store (.ai-memory/details.jsonl). */
function detailStorePath(root) { return path.join(root, "details.jsonl"); }

/** Append one detail body to the consolidated store as {id, body}. */
function appendDetailToStore(root, id, body) {
  const store = detailStorePath(root);
  fs.mkdirSync(path.dirname(store), { recursive: true });
  fs.appendFileSync(store, JSON.stringify({ id, body }) + "\n", "utf8");
}

/** Read a detail body by id from the consolidated store (last write wins). */
function readDetailFromStore(root, id) {
  if (!id) return null;
  let txt;
  try { txt = fs.readFileSync(detailStorePath(root), "utf8"); } catch { return null; }
  let found = null;
  for (const line of txt.split("\n")) {
    if (!line.trim()) continue;
    try { const o = JSON.parse(line); if (o.id === id && typeof o.body === "string") found = o.body; } catch { /* skip */ }
  }
  return found;
}

/** Remove all detail lines for an id from the consolidated store. */
function removeDetailFromStore(root, id) {
  const store = detailStorePath(root);
  let txt;
  try { txt = fs.readFileSync(store, "utf8"); } catch { return false; }
  const kept = [];
  let removed = false;
  for (const line of txt.split("\n")) {
    if (!line.trim()) continue;
    try { if (JSON.parse(line).id === id) { removed = true; continue; } } catch { /* keep unparseable */ }
    kept.push(line);
  }
  if (removed) fs.writeFileSync(store, kept.length ? kept.join("\n") + "\n" : "", "utf8");
  return removed;
}

/**
 * One-time, idempotent migration from the legacy per-entry sidecars
 * (details/<id>.md) to the consolidated store (details.jsonl). Bodies already
 * in the store are not duplicated; old files are removed best-effort (a
 * read-only mount may refuse — readDetail still falls back to them).
 */
function migrateDetailsToStore(root) {
  const dir = path.join(root, "details");
  ...
  const have = new Set();
  try {
    const txt = fs.readFileSync(detailStorePath(root), "utf8");
    for (const line of txt.split("\n")) { if (!line.trim()) continue; try { have.add(JSON.parse(line).id); } catch { /* skip */ } }
  } catch { /* no store yet */ }
  let out = "";
  let migrated = 0;
  for (const f of files) {
    ...
    out += JSON.stringify({ id, body }) + "\n";
    migrated++;
  }
  if (out) { try { fs.appendFileSync(detailStorePath(root), out, "utf8"); } catch { return 0; } }
  for (const f of files) { try { fs.unlinkSync(path.join(dir, f)); } catch { /* mount may forbid */ } }
  try { fs.rmdirSync(dir); } catch { /* non-empty or forbidden */ }
  return migrated;
}
...
export function appendEntry(cwd, entry) {
  ...
  if (!_migratedRoots.has(p.root)) { _migratedRoots.add(p.root); try { migrateDetailsToStore(p.root); } catch { /* best-effort */ } }

  // Tier-2: if the caller passed a rich `detail` body, store it as a sidecar
  // file (details/<id>.md) and replace it with a lightweight `detailRef`
  // pointer. This keeps sessions.jsonl (and every mirror of it) one-line-per-
  // entry, so readEntries stays cheap and the body only loads via readDetail().
  // The id is resolved up front so the sidecar name is deterministic.
  const id  = entry.id || `amp_${generateULID()}`;
  const e2  = { ...entry, id };
  if (typeof entry.detail === "string" && entry.detail.trim()) {
    try {
      appendDetailToStore(p.root, id, entry.detail);
      e2.detailRef = "details.jsonl";
    } catch { /* detail store is best-effort; the entry itself still lands */ }
  }
  delete e2.detail; // never inline the body on the wire

  const dest = destinationFile(p, e2);
  const amp  = toAmp(e2);
...
/**
 * Absolute path to an entry's Tier-2 sidecar body file, given its id.
 * (details/<id>.md under the active memory root.)
 */
export function detailPath(cwd, id) {
  const p = ampPaths(cwd);
  return path.join(p.root, "details", `${id}.md`);
}

/**
 * Load the Tier-2 detail body for an entry, on demand. Accepts an internal- or
 * AMP-shape entry; resolves its `detailRef` (internal) or `meta.detailRef`
 * (wire). Returns the body string, or null if the entry has no detail or the
 * sidecar file is missing. This is the ONLY path that reads the rich body —
 * readEntries deliberately never loads it, so injection stays lean.
 */
export function readDetail(cwd, entry) {
  if (!entry) return null;
  const ref = entry.detailRef || (entry.meta && entry.meta.detailRef);
  if (!ref) return null;
  const p  = ampPaths(cwd);
  const id = entry.id || (entry.meta && entry.meta.id);
  // Consolidated store first (current layout).
  const fromStore = readDetailFromStore(p.root, id);
  if (fromStore != null) return fromStore;
  // Back-compat: legacy per-entry sidecar file (details/<id>.md).
  if (typeof ref === "string" && ref !== "details.jsonl") {
    const abs = path.isAbsolute(ref) ? ref : path.join(p.root, ref);
...
export function deleteEntry(cwd, id) {
  ...
  // Clean up the Tier-2 sidecar body, if any — otherwise a forgotten entry
  // leaves its (potentially large) detail file orphaned on disk.
  try { removeDetailFromStore(p.root, id); } catch { /* best-effort */ }
  try {
    const abs = path.join(p.root, "details", `${id}.md`);
    if (fs.existsSync(abs)) { fs.unlinkSync(abs); touched.push(abs); }
  } catch { /* best-effort */ }
...
export function pruneEntries(cwd, opts = {}) {
  ...
  for (const file of files) {
    ...
    for (const line of lines) {
      ...
      // Bookmarks are named resume points — never age them out, even though
      // they're `note`-typed and would otherwise be archivable.
      const isBookmark = Array.isArray(entry.tags) && entry.tags.includes("bookmark");
      const archivable = archivableTypes.has(type) && !PROTECTED_TYPES.has(type) && !isBookmark;
  ...
  const data = {
    ...
    config: {
      ...
      injection: {
        ...
        maxCommits: 5,
        maxEntryChars: 200,
        // "compact" = ~3-line protocol summary (default; the full trigger table
        // is redundant with the amp_* tool descriptions). "full" restores the
        // table; "off" omits the protocol block entirely.
