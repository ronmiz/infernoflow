...
function checkMemoryProtocol(cwd) {
  const files = [".cursorrules", "CLAUDE.md", path.join(".github", "copilot-instructions.md")];
  const present = [];
  ...
  if (present.length) {
    return pass(`Memory protocol in ${present.length}/${files.length} rule file(s) — AI knows when to auto-capture`);
  }
  return warn(
    "Memory protocol missing from rule files — the AI won't auto-capture gotchas",
    "infernoflow refresh   (writes the amp_write capture protocol into CLAUDE.md / .cursorrules)",
  );
}
...
function checkStaleNpmScripts(cwd) {
  ...
  const VALID_VERBS = new Set([
    "log", "ask", "switch", "recap", "status", "init", "doctor",
    "graph", "watch", "amp", "contract", "dev", "demo", "setup",
    "log-decision", "log-attempt", "context", "stats", "test",
  ]);
