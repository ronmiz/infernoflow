...
function checkCloudToken() {
  ...
  try {
    ...
    if (!creds?.access_token) {
      return { status: "info", message: "Credentials file present but no token", fix: "infernoflow login", detail: null };
    }
    if (creds.expires_at) {
      const expiresAt = new Date(creds.expires_at).getTime();
      if (Date.now() > expiresAt) return warn("Cloud token expired", "infernoflow login");
    }
    const who = creds.user?.login || creds.user?.name || "unknown";
    return pass(`Logged in as ${who} (cloud sync active)`);
  } catch {
