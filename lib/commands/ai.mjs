...
async function testProvider(providerId, config, cwd) {
  try {
    // KNOWN BUG: providerRouter no longer exports callAI (only resolveProvider
    // + detectAvailableProviders). `infernoflow ai test` is broken in 0.44.x
    // and throws "callAI is not a function" at runtime. The surrounding
    // try/catch swallows the failure so the command at least doesn't crash —
    // it just returns null. Tracked for 0.44.2 / 0.45 (restore callAI or
    // strip the `ai test` subcommand).
    const mod = /** @type {{ callAI?: (p: string, cwd: string, id: string) => Promise<any> }} */ (await import("../ai/providerRouter.mjs"));
    if (typeof mod.callAI !== "function") return null;
    const testPrompt = `Reply with exactly: "infernoflow AI test OK — ${providerId}"`;
    const result = await mod.callAI(testPrompt, cwd, providerId);
    return result;
  } catch {
    return null;
  }
