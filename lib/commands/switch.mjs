...
function getRecentSessions(count = 8) {
...
function buildHandoff(toAgent) {
  ...
  const themeChanges = sessions.filter(e => e.type === "theme");
  const recent      = sessions.slice(-5); // last 5 regardless of type
  ...
  const failed = attempts.filter(e => e.result === "failed" || e.result === "partial");
  if (failed.length) {
...
export async function switchCommand(args) {
  ...
  const handoff = buildHandoff(toAgent);
  ...
  if (jsonFlag) {
    ...
    const theme    = readJSON(THEME_FILE);
    const sessions = getRecentSessions(8);
    console.log(JSON.stringify({ state, contract: { policyId: contract.policyId, policyVersion: contract.policyVersion, capabilities: contract.capabilities }, theme, sessions, generatedAt: new Date().toISOString() }, null, 2));
    return;
  }
  ...
  // Print preview
  const sessionCount = getRecentSessions(100).length;
  const state    = readJSON(STATE_FILE) || {};
  const theme    = readJSON(THEME_FILE);
  const contract = readJSON(CONTRACT_FILE) || {};
  ...
  console.log("  Sessions      " + sessionCount + " entries in memory");
