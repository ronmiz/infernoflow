...
import { ampPaths, readEntries as ampRead, appendEntry as ampAppend, readDetail } from "../amp/io.mjs";
...
function buildHandoff(toAgent, sinceArg, allFlag) {
  ...
  const bookmarks = pool.filter(e => Array.isArray(e.tags) && e.tags.includes("bookmark"));
  ...
  // ── Bookmarks — named resume points; newest context inlined ───────────────
  // High-value "return here" markers the dev dropped this session. We list all,
  // then inline the MOST RECENT bookmark's captured context (Tier-2 detail) so
  // the next session opens right where work paused — without bloating the rest.
  if (bookmarks.length) {
    lines.push(`## 🔖 Bookmarks — Resume Points (${bookmarks.length})`, "");
    bookmarks.forEach((e, i) => lines.push(`${i + 1}. **${e.summary}**  _(${fmtDate(e.ts)})_`));
    const latest = bookmarks[bookmarks.length - 1];
    const body = latest ? readDetail(process.cwd(), latest) : null;
    if (body && body.trim()) {
      lines.push("", `<details><summary>📎 Context for "${latest.summary}"</summary>`, "", body.trim(), "", "</details>");
    }
    lines.push("");
  }
