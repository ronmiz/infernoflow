...
import { header, ok, warn, info, done, nextSteps, bold, cyan, yellow, gray, green, red } from "../ui/output.mjs";
import {
...
import { applyCleanTreePolicy } from "../cleanTree.mjs";
...
function installSkills(cwd, force) {
  // Ship the infernoflow Claude Code skill(s) into the project so the coding
  // agent proactively logs gotchas/decisions and drops bookmarks. Idempotent:
  // existing files are left alone unless --force. Quiet on skips.
  const src = path.join(getTemplatesRoot(), "skills");
  ...
  const walk = (s, d) => {
    ...
    for (const e of fs.readdirSync(s, { withFileTypes: true })) {
      ...
      if (fs.existsSync(dp) && !force) continue;
      fs.mkdirSync(path.dirname(dp), { recursive: true });
      fs.copyFileSync(sp, dp);
      created++;
      ok("Created: " + cyan(path.relative(cwd, dp)));
    }
  };
  walk(src, path.join(cwd, ".claude", "skills"));
  return created;
}

function installAgents(cwd, force) {
  // Ship the infernoflow subagent(s) (e.g. memory-keeper) into the project so
  // the coding agent can delegate memory capture. Idempotent; quiet on skips.
  // Passive: an agent file only acts when explicitly invoked.
  const src = path.join(getTemplatesRoot(), "agents");
  if (!fs.existsSync(src)) return 0;
  let created = 0;
  const walk = (s, d) => {
    fs.mkdirSync(d, { recursive: true });
  ...
  walk(src, path.join(cwd, ".claude", "agents"));
  return created;
}

function printClaudeAssetsHint() {
  console.log("");
  console.log("  " + yellow("↻ Restart Claude Code / Cursor") + gray(" to activate the infernoflow skill + memory-keeper agent."));
  console.log("  " + gray("Editors only load newly-added .claude/ skills & agents on restart. Verify with ") + cyan("/skills") + gray(" and ") + cyan("@agent-") + gray("."));
}
...
export function ensureGitignoreEntries(cwd, { silent = false } = {}) {
...
async function initLite(cwd, force) {
  ...
  console.log("  " + cyan("infernoflow init --mode full") + gray(" — expand to full setup when you need it"));
  console.log();
}
...
async function initMemory(cwd, force, yes) {
  ...
  // "Already set up" must mean a real memory STORE exists — consistent with
  // what `amp status` reports. The old check used `exists(ampDir) || exists(legacyDir)`,
  // so a bare `inferno/` dir (e.g. contract-only, no sessions.jsonl) made init
  // print "already set up" while `amp status` said "not initialised" and the
  // entries count stayed 0 — the store wasn't created until the first `log`.
  // Anchoring on the sessions file instead means a half-state falls through to
  // fresh setup below, which actually creates `.ai-memory/`.
  const ampSessionsExist    = fs.existsSync(sessionsFile);
  const legacySessionsExist = fs.existsSync(path.join(legacyDir, "sessions.jsonl"));
  const alreadyInit = ampSessionsExist || legacySessionsExist;
  if (alreadyInit && !force) {
    ...
    // Migrate to v0.44 clean-tree policy: rule files tracked, only
    // global.jsonl gitignored, branches/*.jsonl get merge=union. The
    // helper also strips the legacy v0.43 "ignore everything" block.
    ...
    const _sk1 = installSkills(cwd, force);
    const _ag1 = installAgents(cwd, force);
    if ((_sk1 + _ag1) > 0) printClaudeAssetsHint();
  ...
  // v0.44 clean-tree policy: writes `.gitignore` and `.gitattributes` managed
  // blocks so branch JSONLs travel via git (teammates inherit branch memory)
  // while `global.jsonl` and regenerated artifacts stay out of the tree.
  applyCleanTreePolicy(cwd);
  ...
  for (const r of ruleResults) {
    if ("created" in r && (r.created || r.updated)) {
      ok((r.created ? "Created: " : "Updated: ") + cyan(r.rel));
    }
  }

  // Install the infernoflow Claude Code skill(s) for proactive capture.
  const _sk2 = installSkills(cwd, force);
  const _ag2 = installAgents(cwd, force);
  if ((_sk2 + _ag2) > 0) printClaudeAssetsHint();
  ...
  if (gotcha) {
    ...
    // Non-interactive (--yes / agent-driven) installs skip the prompt above
    // and would otherwise finish with an empty memory store. Drop a visible
    // demo entry so the user (or the agent verifying setup) can confirm the
    // loop is wired end-to-end with one command: `infernoflow status`.
    ampAppend(cwd, {
      ts: new Date().toISOString(),
      agent: "infernoflow",
      type: "note",
      summary: "infernoflow init complete — memory loop is live. Run `infernoflow status` to verify.",
      source: "init",
    });
  }

  console.log("\n  " + green("✔") + " You're set up. Quick commands:\n");
  console.log("  " + cyan("infernoflow status") + gray("        — verify the loop end-to-end"));
  console.log("  " + cyan("infernoflow log \"...\"") + gray("    — remember something"));
...
export async function initCommand(args) {
  ...
  if (templateName) {
    // Legacy --mode full --template path. The templates/index.mjs module
    // was cut in v0.44 along with the rest of the capability-contract
    // scaffolding; this block will be removed entirely in v0.45. Until
    // then the try/catch makes it fail soft.
    /** @type {undefined | { getTemplate: (n: string) => any; listTemplates: () => Array<{ name: string }> }} */
    let tmplMod;
    // @ts-expect-error — module deleted in v0.44; intentionally unresolved.
  ...
  // v0.44.3: full mode used to seed a generic todo-app contract (CreateTask,
  // ReadTasks, UpdateTask, …) unrelated to the project — users had to discover
  // and rewrite it by hand. Run adoption detection (scan the real codebase)
  // whenever we're in full mode, so capabilities reflect THIS project. Explicit
  // --adopt still works; the only path that kept the placeholder was the silent
  // `--mode full --yes` one, which now adopts too.
  const autoAdopt = adopt || isFullMode;
  if (autoAdopt) {
    ...
    if (!yes && !reportJsonOnly) {
      const rl = readline.createInterface({ input: process.stdin, output: process.stdout });
      /** @type {{ detected?: { language?: string; framework?: string; projectType?: string }; language?: string; framework?: string; projectType?: string }} */
    ...
    // Adoption found nothing concrete (small/empty/unfamiliar codebase). Seed a
    // single clearly-labelled placeholder rather than a misleading todo-app set,
    // and tell the user to fill it in — `check` still passes (it gets a scenario).
    if (capabilities.length === 0) {
      capabilities = ["ExampleCapability"];
      if (!silent) warn("No capabilities detected from the code — wrote a placeholder. Edit " + cyan("inferno/capabilities.json") + " to describe what this project does.");
    }
  } else if (!yes) {
    const rl = readline.createInterface({ input: process.stdin, output: process.stdout });
