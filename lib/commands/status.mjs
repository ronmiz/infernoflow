...
export async function statusCommand(args = []) {
  ...
  if (isMemoryMode) {
    // Use the branch-aware merged reader so entries written to
    // .ai-memory/branches/<branch>.jsonl (per-branch) and
    // .ai-memory/global.jsonl (personal) are counted — not just the
    // legacy flat sessions.jsonl. Without this, the per-slice memory
    // captured during dogfood doesn't show up in `infernoflow status`.
    ...
    // Only nudge toward full-mode contracts if they have NEVER been set up.
    // Previously this hint always printed in memory-mode — including AFTER
    // the user had already run `init --mode full`, because amp.json + contract.json
    // can coexist. The hint stays helpful for new users and stops nagging
    // experienced ones who've already made the contract decision.
