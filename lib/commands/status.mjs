...
export async function statusCommand(args = []) {
  ...
  const configPath  = path.join(infernoDir, "config.json");
  const contractPath = path.join(infernoDir, "contract.json");
  const mode = (() => {
    try { return JSON.parse(fs.readFileSync(configPath, "utf8")).mode || null; } catch { return null; }
  })();
  const isMemoryMode = (mode === "memory") || (!fs.existsSync(contractPath) && fs.existsSync(configPath));

  if (isMemoryMode) {
    const sessionsPath = path.join(infernoDir, "sessions.jsonl");
    const entries = (() => {
