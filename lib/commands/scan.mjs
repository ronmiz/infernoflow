...
// ── UI element detection (JSX / TSX only) ────────────────────────────────────
//
// Walks JSX source for interactive elements (button, input, form, a, select)
// and captures their handler + label. Each becomes a "UI node" in the graph,
// edged to whatever capability owns its handler function.
//
// Multi-line JSX attributes are common, so we use [\s\S]*? (newline-tolerant
// non-greedy match) instead of [^>]* (which fails on newlines).
const UI_PATTERNS = [
  // <button ... onClick={handler} ...>Label
  { tag: "button", re: /<button[\s\S]*?on(?:Click|Press|Submit)\s*=\s*\{?([A-Za-z_$][\w$]*)\}?[\s\S]*?>([\s\S]*?)<\/button>/gi },
  // <input ... onChange={handler} ... />     (and other input event names)
  { tag: "input",  re: /<input[\s\S]*?on(?:Change|Input|Blur)\s*=\s*\{?([A-Za-z_$][\w$]*)\}?[\s\S]*?\/?>/gi },
  // <form ... onSubmit={handler}>
  { tag: "form",   re: /<form[\s\S]*?onSubmit\s*=\s*\{?([A-Za-z_$][\w$]*)\}?[\s\S]*?>/gi },
  // <a ... onClick={handler}>Label
  { tag: "link",   re: /<a[\s\S]*?onClick\s*=\s*\{?([A-Za-z_$][\w$]*)\}?[\s\S]*?>([\s\S]*?)<\/a>/gi },
  // <select ... onChange={handler}>
  { tag: "select", re: /<select[\s\S]*?onChange\s*=\s*\{?([A-Za-z_$][\w$]*)\}?[\s\S]*?>/gi },
];

function detectUiElements(text, filePath) {
  if (!/\.(jsx|tsx|vue|svelte)$/i.test(filePath)) return [];
  const elements = [];
  for (const { tag, re } of UI_PATTERNS) {
    const r = new RegExp(re.source, re.flags);
    let m;
    while ((m = r.exec(text)) !== null) {
      const handler = m[1];
      // Strip JSX expressions, whitespace from inner content
      const rawLabel = (m[2] || "").replace(/\{[^}]*\}/g, "").replace(/\s+/g, " ").trim().slice(0, 40);
      elements.push({
        tag,
        handler,
        label: rawLabel || handler,
        file:  filePath,
      });
      if (elements.length >= 50) return elements; // safety cap per file
    }
  }
  return elements;
}

// ── Component detection (JSX / TSX) ──────────────────────────────────────────
//
// React-style function components: name starts with capital letter and
// returns JSX. We don't AST-parse for speed — a handful of regexes catches
// 95% of cases (export default function X, export function X, const X = (...) =>).
const COMPONENT_PATTERNS = [
  /export\s+default\s+function\s+([A-Z][\w$]*)/g,
  /export\s+function\s+([A-Z][\w$]*)/g,
  /^function\s+([A-Z][\w$]*)\s*\([\s\S]*?\)\s*\{[\s\S]*?return\s*\(/gm,
  /(?:export\s+)?const\s+([A-Z][\w$]*)\s*=\s*(?:\([\s\S]*?\)|[\w$]+)\s*=>\s*[({<]/g,
];

function detectComponents(text, filePath) {
  if (!/\.(jsx|tsx|vue|svelte)$/i.test(filePath)) return [];
  const found = new Set();
  ...
  // Find which OTHER components are rendered inside this file's JSX.
  // Match `<CapitalizedName` (excluding self-references). Stops at `>` or
  // whitespace so attributes don't interfere. Skips HTML tags (lowercase
  // ones don't match the leading-capital rule).
  const usages = new Set();
  const usageRe = /<([A-Z][\w$]*)/g;
  let um;
  while ((um = usageRe.exec(text)) !== null) {
    const name = um[1];
    if (name && !found.has(name)) usages.add(name); // skip own definitions
  }

  return [...found].map(name => ({
    name,
    file: filePath,
    renders: [...usages],   // component names this file uses in JSX
  }));
}
...
export async function scanCommand(rawArgs) {
  ...
  // ── UI element + component detection (JSX / TSX / Vue / Svelte) ─────────
  // Walk the same files for components AND interactive elements.
  //   - Components: React function components by Capitalized name.
  //   - UI elements: buttons, inputs, forms, links, selects with handlers.
  // The graph layer wires elements → components → capabilities.
  const uiElements = [];
  const components = [];
  for (const filePath of files) {
    if (!/\.(jsx|tsx|vue|svelte)$/i.test(filePath)) continue;
    try {
      const text   = fs.readFileSync(filePath, "utf8");
      const relRaw = path.relative(cwd, filePath).replace(/\\/g, "/");
      uiElements.push(...detectUiElements(text, relRaw));
      components.push(...detectComponents(text, relRaw));
    } catch { /* skip unreadable */ }
  }
  if (!jsonMode) {
    if (components.length > 0) console.log(`  Found ${components.length} components (React/Vue/Svelte).`);
    if (uiElements.length > 0) console.log(`  Found ${uiElements.length} UI elements (buttons, inputs, forms, links).`);
  }
  ...
  const scanData = {
    ...
    // UI elements as a separate top-level array. Each: {tag, handler, label, file}.
    // The graph command wires each handler back to the capability that owns it.
    uiElements: uiElements,
    // Components (React/Vue/Svelte function components): {name, file}.
    // The graph treats these as a tier between UI elements and capabilities.
