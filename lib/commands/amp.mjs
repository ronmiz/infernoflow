...
function readRawEntries(cwd) {
  ...
  if (!fs.existsSync(sessions)) return { sessions, entries: [], lines: [] };
  const lines = fs.readFileSync(sessions, "utf8").split("\n").filter(Boolean);
  const entries = lines.map((l, i) => {
    try { return { ok: true, line: i + 1, value: JSON.parse(l) }; }
    catch (err) { return { ok: false, line: i + 1, value: null, error: err.message }; }
  });
  return { sessions, entries, lines };
}
...
function doValidate(cwd) {
  ...
  for (const e of entries) {
    if (!e.ok) {
      parseErrors++;
      failures.push({ line: e.line, error: `parse error: ${e.error}` });
      continue;
    }
    const errs = validateAmpEntry(e.value, e.line);
    if (errs.length) {
      schemaErrors++;
      failures.push({ line: e.line, error: errs.join("; ") });
    } else {
      ok++;
    }
  }
  ...
  for (const f of failures.slice(0, 10)) {
    console.log(`  ${red("✘")} line ${f.line}  ${gray(f.error)}`);
  }
