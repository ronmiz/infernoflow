...
export async function ampCommand(rawArgs) {
  ...
  // ── AMP-spec verb aliases ──────────────────────────────────────────────────
  // The MCP server advertises amp_read / amp_write / amp_search / amp_handoff /
  // amp_health, but the CLI `amp` namespace only had status/migrate/validate —
  // so anyone scanning the MCP tool list and trying `infernoflow amp read` hit
  // "unknown verb". These thin aliases route to the same handlers the bare
  // commands use, so the CLI and MCP surfaces match name-for-name.
  // NOTE: every CLI handler is invoked by bin as `handler([cmd, ...rest])` and
  // treats args[0] as its own command name (logCommand/askCommand skip it). So
  // we must prepend the canonical command token, or the first real argument
  // (e.g. the log message) gets eaten as if it were the command name.
  if (verb === "write")  return (await import("./log.mjs")).logCommand(["log", ...rest]);
  if (verb === "read" || verb === "search") return (await import("./ask.mjs")).askCommand(["ask", ...rest]);
  if (verb === "handoff") return (await import("./switch.mjs")).switchCommand(["switch", ...rest]);
  if (verb === "health")  return (await import("./recap.mjs")).recapCommand(["recap", "--brief", ...rest]);

  console.error(red(`\n  Unknown amp verb: ${verb}`));
  console.error(gray(`  Try:  infernoflow amp  (status | migrate | validate | version | read | write | search | handoff | health)\n`));
  process.exit(1);
}
