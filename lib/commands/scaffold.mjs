...
function toKebabCase(id) {
  return id
    ...
    .toLowerCase()
    .replace(/[_\s]+/g, "-")                   // snake/space → hyphen
    .replace(/^-+|-+$/g, "");
}

/**
 * Normalize a user-supplied capability ID to the canonical PascalCase form
 * the rest of the system uses (matching what scan + adopt produce in
 * inferno/capabilities.json). Accepts: kebab-case, snake_case, camelCase,
 * PascalCase, or space-separated.
 *
 * Returns null if the input can't be parsed into at least one identifier-shaped
 * token (e.g. starts with a digit, contains only punctuation).
 */
function normalizeCapabilityId(input) {
  if (!input || typeof input !== "string") return null;
  const trimmed = input.trim();
  // Reject anything with characters we'd never want in an ID
  if (!/^[A-Za-z][A-Za-z0-9 _-]*$/.test(trimmed)) return null;
  // Split on any combination of separators OR camel/Pascal boundaries
  const parts = trimmed
...
export async function scaffoldCommand(rawArgs) {
  ...
  let capId = args.find((a, i) => !a.startsWith("--") && !skipIdxs.has(i));

  if (!capId) {
    console.error(red("✗ Usage: infernoflow scaffold <capability-id> [--dir <src>] [--lang ts|js|py|go] [--dry-run] [--json]"));
    console.error(gray("  Example: infernoflow scaffold CreateItem   (or payment-refund — both work)"));
    process.exit(1);
  }

  // Normalize cap ID to canonical PascalCase. The rest of the system
  // (scan, adopt, contract.json) uses PascalCase IDs, so we match.
  // Accepts kebab/snake/camel/Pascal/space-separated input.
  const inputId  = capId;
  const normalId = normalizeCapabilityId(inputId);
  if (!normalId) {
    console.error(red(`✗ Invalid capability ID: "${inputId}"`));
    console.error(gray("  Try: CreateItem, payment-refund, user_auth, or \"Send Email\"."));
    process.exit(1);
  }
  capId = normalId; // canonical from here on
  ...
  // Check duplicate against either the canonical or the user-typed form
  if (allCaps.some(c => c.id === capId || c.id === inputId)) {
