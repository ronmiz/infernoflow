...
import { bold, cyan, gray, green, yellow, red, orange } from "../ui/output.mjs";
...
function formatEntry(e, i) {
  ...
  // Route through the gated color helpers (output.mjs honors non-TTY / NO_COLOR
  // / --no-color) instead of hardcoding raw \x1b[..m escapes, which leaked
  // literal SGR codes like "[36m" into piped / agent-captured output.
  const paintType = type === "gotcha"     ? yellow
                  : type === "decision"   ? cyan
                  : type === "theme"      ? orange
                  : type === "preference" ? cyan
                  : type === "attempt"    ? gray
                  : type === "error"      ? red
                  :                         (s) => s;
  const result = e.result ? ` [${e.result}]` : "";
  const agent  = e.agent && e.agent !== "human" ? gray(` (${e.agent})`) : "";
  return `  ${gray(String(i+1).padStart(3))}  ${gray(ts)}  ${paintType(type)}${result}  ${e.summary}${agent}`;
}
...
export async function logCommand(args) {
  ...
  // --no-color forces plain output for this invocation (agents / pipes). Set
  // before any painting; output.mjs reads NO_COLOR per-call so this takes effect.
  ...
  // ── Clear mode ──────────────────────────────────────────────────────────────
  if (clearFlag) {
    // Branch-aware clear. Since v0.44 entries live in per-branch files
    // (branches/<branch>.jsonl) and are only MIRRORED into the legacy flat
    // sessions.jsonl. Renaming just sessions.jsonl left every branch-routed
    // entry visible via the merged reader — so --clear cleared nothing on a
    // modern project. Archive every project memory source instead. Personal
    // global.jsonl is intentionally left alone: --clear is for the session log,
    // not your cross-project preferences.
    const p = resolvedPaths();
    const stamp = Date.now();
    const targets = new Set([p.sessions, p.currentBranchFile, p.defaultBranchFile].filter(Boolean));
    try {
      if (p.branchesDir && fs.existsSync(p.branchesDir)) {
        for (const name of fs.readdirSync(p.branchesDir)) {
          if (name.endsWith(".jsonl")) targets.add(path.join(p.branchesDir, name));
        }
      }
    ...
    for (const file of targets) {
      ...
      const archive = file.replace(/\.jsonl$/, `-archive-${stamp}.jsonl`);
      try { fs.renameSync(file, archive); archived.push(path.basename(archive)); }
    ...
    console.log(green(`  ✔ Session log archived (${archived.length} file${archived.length === 1 ? "" : "s"}) — entries cleared from active memory.\n`));
    return;
  }
  ...
  // Tier-2 rich body: --detail "<text>" for short bodies, or --detail-file <path>
  // (use "-" for stdin) for large ones — e.g. `... | infernoflow log "snapshot"
  // --type note --detail-file -`. Stored in a sidecar (details/<id>.md), never
  // inlined into the injected rule-file block, so 'msg' stays the lean summary.
  const detailArg     = flag("--detail",      null);
  const detailFileArg = flag("--detail-file", null);
  let detail = null;
  if (detailFileArg) {
    try {
      detail = detailFileArg === "-"
        ? fs.readFileSync(0, "utf8")            // stdin
        : fs.readFileSync(detailFileArg, "utf8");
    } catch (err) {
      if (!quietFlag) console.error(red(`  ✘ Could not read --detail-file ${detailFileArg}: ${err.message}\n`));
      process.exit(1);
    }
  } else if (detailArg) {
    detail = detailArg;
  ...
  if (!quietFlag) {
    ...
    const detailLabel = detail && detail.trim() ? gray(` (+${detail.length}-char detail)`) : "";
    console.log(green(`  ✔ Logged${typeLabel}${resultLabel}${sourceLabel}: `) + summary + detailLabel + "\n");
  }
}
