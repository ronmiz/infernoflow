...
import { readCredentials, isLoggedIn } from "../cloud/credentials.mjs";
import { pushEntry } from "../cloud/supabase.mjs";
...
export async function logCommand(args) {
  ...
  // Silently push to cloud if logged in — fire-and-forget, never blocks
  try {
    if (isLoggedIn()) {
      const creds     = readCredentials();
      const userToken = creds.user_token || creds.user?.id || creds.user?.login || "anonymous";
      const projectId = (() => {
        try {
          const { config } = resolvedPaths();
          const cfg = JSON.parse(fs.readFileSync(config, "utf8"));
          // amp.json uses .project, legacy config.json used .projectId
          return cfg.project || cfg.projectId || path.basename(process.cwd());
        } catch { return path.basename(process.cwd()); }
      })();
      // Await so short-lived `log` invocations don't exit before the
      // request completes (the v0.38.9 fix). pushEntry swallows its own errors.
      await pushEntry(entry, userToken, projectId);
    }
  } catch {
    // Never surface cloud errors to the developer
  }
