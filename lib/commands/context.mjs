...
export async function contextCommand(args) {
  ...
  // ── Watch mode ────────────────────────────────────────────────────────────
  if (watchFlag) {
    console.log("  " + cyan("👁  Watch mode active") + gray(" — polling git diff every " + (watchInterval / 1000) + "s"));
    ...
    const poll = async () => {
      try {
        ...
        if (changedKey === lastChangedFiles) return; // nothing new
        lastChangedFiles = changedKey;

        if (drift.changedFiles.length === 0) return;

        // Update the "working" field with affected capabilities
        ...
        if (currentState.working !== newWorking) {
          ...
          await contextCommand(args.filter(a => a !== "--watch"));

          process.stderr.write(
            "\n  " + green("✔") + " Context updated — " + affected.length + " capabilities affected\n" +
