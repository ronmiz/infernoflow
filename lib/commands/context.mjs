...
function copyToClipboard(text){try{const p=process.platform;if(p==="win32")execSync("clip",{input:text});else if(p==="darwin")execSync("pbcopy",{input:text});else{try{execSync("xclip -selection clipboard",{input:text})}catch{execSync("xsel --clipboard --input",{input:text})}}return true;}catch{return false;}}
