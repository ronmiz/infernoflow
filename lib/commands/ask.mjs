...
import { readEntries as ampRead } from "../amp/io.mjs";

// (Dead INFERNO_DIR / SESSIONS_FILE constants removed in v0.44.1 — `ask`
//  reads via ampRead so it picks up the branch-aware merged layout.)
...
function fmtRelDate(iso) {
  ...
  // Calendar-day difference (local midnight to local midnight), NOT
  // elapsed-ms / 86400000. The old rolling-24h math labeled a 25-hour-old entry
  // "yesterday" mid-session and drifted as "now" advanced — so entries written
  // earlier in the SAME session could read "yesterday" while later ones read
  // "today", making "where did I leave off?" hard to reason about. Same-day
  // entries also get an absolute HH:MM anchor so the newest action is obvious.
  ...
  if (queryTokens.length > 0) {
    scored = candidates
      ...
      .sort((a, b) => {
        ...
        return new Date(b.entry.ts || 0).getTime() - new Date(a.entry.ts || 0).getTime();
      });
  } else {
...
export async function askCommand(rawArgs = []) {
  ...
  // args[0] is the command name ("ask") — skip it.
  const positional = args.slice(1);
  const queryWords = positional.filter((a, i) => {
    if (a.startsWith("--")) return false;
    if (i > 0 && positional[i - 1].startsWith("--")) return false; // flag value
    return true;
  });
