...
export function updateClaudeDesktopConfig(mcpServerAbsPath, projectDir) {
  ...
  if (existing && Array.isArray(existing.args) && existing.args[0] === mcpServerAbsPath
      && existing.env && existing.env.INFERNOFLOW_PROJECT_DIR === projectDir) {
    return { updated: false, backup };
  }
  const env = { ...(existing && existing.env ? existing.env : {}) };
  if (projectDir) env.INFERNOFLOW_PROJECT_DIR = projectDir;
  config.mcpServers.infernoflow = { command: "node", args: [mcpServerAbsPath], env };
...
export function updateClaudeJson(mcpServerAbsPath, projectDir) {
...
export function updateVscodeMcpJson(cwd, mcpServerAbsPath, projectDir) {
  ...
  if (current && current.args && current.args[0] === mcpServerAbsPath
      && current.env && current.env.INFERNOFLOW_PROJECT_DIR === projectDir) {
    return { updated: false, backup };
  }

  const env = { ...(current && current.env ? current.env : {}) };
  if (projectDir) env.INFERNOFLOW_PROJECT_DIR = projectDir;
...
export function autoSetupMcp(cwd, { silent = false } = {}) {
  ...
  try {
    const r = updateClaudeJson(dstMcp, projectDir);
    if (r.updated) {
  ...
  try {
    const r = updateVscodeMcpJson(cwd, dstMcp, projectDir);
    if (r.updated) {
  ...
  try {
    ...
    if (!cur || !cur.args || cur.args[0] !== dstMcp || !cur.env || cur.env.INFERNOFLOW_PROJECT_DIR !== projectDir) {
      cursorCfg.mcpServers.infernoflow = { command: "node", args: [dstMcp], env: { ...(cur && cur.env ? cur.env : {}), INFERNOFLOW_PROJECT_DIR: projectDir } };
  ...
  try {
    const r = updateClaudeDesktopConfig(dstMcp, projectDir);
    if (r.updated) {
