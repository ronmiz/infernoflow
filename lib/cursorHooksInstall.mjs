...
export function installCursorHooksArtifacts(opts) {
  ...
  // MCP server — keep it inside .cursor/ so we don't pollute the project root.
  // Earlier versions copied it to ./inferno-mcp-server.mjs which made other
  // contributors / reviewers complain about a stray top-level file.
  ...
  // .cursor/mcp.json — point to the in-folder MCP server
