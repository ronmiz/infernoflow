...
import { getOAuthUrl, getUser, SUPABASE_URL, SUPABASE_ANON_KEY } from "../cloud/supabase.mjs";
...
async function doLogin() {
  ...
  if (SUPABASE_URL.includes("YOUR_PROJECT")) {
    ...
    console.log(`  ${gray("1.")} Create a project at ${cyan("https://supabase.com")}`);
    console.log(`  ${gray("2.")} Run ${cyan("scripts/supabase-schema.sql")} in the SQL editor`);
    console.log(`  ${gray("3.")} Enable GitHub OAuth in Authentication → Providers`);
    console.log(`  ${gray("4.")} Set ${cyan("INFERNOFLOW_SUPABASE_URL")} and ${cyan("INFERNOFLOW_SUPABASE_ANON_KEY")}`);
    console.log(`     (or hardcode them in lib/cloud/supabase.mjs)`);
    console.log();
    process.exit(1);
  }
  ...
  const server = http.createServer((req, res) => {
    ...
    // Supabase sends the token as a hash fragment (#access_token=...) which
    // the browser can't forward directly. We serve a tiny page that reads the
    // fragment and POSTs it back.
    if (req.method === "GET") {
...
<script>
  ...
  server.listen(PORT, "127.0.0.1", () => {
    const oauthUrl = getOAuthUrl(state, CALLBACK_URI);
