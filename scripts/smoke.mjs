...
try {
  ...
  assert(
    existsSync(join(tmp, ".ai-memory", "amp.json")) || existsSync(join(tmp, "inferno", "config.json")),
    "init did not create .ai-memory/amp.json (new) or inferno/config.json (legacy)"
  );
  ...
  const ampHandoff    = join(tmp, ".ai-memory", "handoff.md");
  const legacyHandoff = join(tmp, "inferno", "HANDOFF.md");
